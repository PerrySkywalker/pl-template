...
class  ModelInterface(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        Y = int(label)
        #Y_hat = int(Y_hat)
        self.accdata[Y]["count"] += 1
        self.accdata[Y]["correct"] += (int(Y_hat.item()) == Y)
    ...
    def validation_epoch_end(self, val_step_outputs):
        ...
        #---->acc log
        for c in range(self.n_classes):
            count = self.accdata[c]["count"]
            correct = self.accdata[c]["correct"]
            if count == 0: 
                acc = None
            else:
                acc = float(correct) / count
    ...
    def test_step(self, batch, batch_idx):
        ...
        Y = int(label)
        #Y_hat = int(Y_hat)
        self.testdata[Y]["count"] += 1
        self.testdata[Y]["correct"] += (int(Y_hat.item()) == Y)
